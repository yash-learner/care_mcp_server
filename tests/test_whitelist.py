...
def test_default_whitelist():
    ...
    assert "facility_create" in allowed
    assert "facility_list" in allowed
    assert "bed_create" in allowed
    assert "users_list" in allowed


def test_is_allowed_for_whitelisted_operation():
    """Test is_allowed returns True for whitelisted operations."""
    manager = WhitelistManager()
    assert manager.is_allowed("facility_create") is True
    assert manager.is_allowed("bed_list") is True
...
def test_is_allowed_blocks_destroy_operations():
    ...
    manager = WhitelistManager()
    assert manager.is_allowed("facility_destroy") is False
    assert manager.is_allowed("bed_destroy") is False


def test_is_allowed_blocks_delete_operations():
    """Test is_allowed blocks operations with _delete pattern."""
    manager = WhitelistManager()
    assert manager.is_allowed("facility_delete") is False
    assert manager.is_allowed("bed_delete") is False
...
def test_custom_whitelist():
    ...
    assert manager.is_allowed("facility_create") is False
...
def test_remove_operation():
    ...
    manager = WhitelistManager()
    manager.remove_operation("facility_create")
