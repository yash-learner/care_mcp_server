...
class Config(BaseSettings):
    ...
    care_base_url: str = Field(
        default="https://careapi.ohc.network", description="Base URL for Care API"
    )

    care_schema_url: Optional[str] = Field(
        default=None, description="Schema URL (defaults to base_url + /api/schema/)"
    )

    # Authentication
    care_username: Optional[str] = Field(
        default=None, description="Username for Care API authentication"
    )

    care_password: Optional[str] = Field(
        default=None, description="Password for Care API authentication"
    )

    care_access_token: Optional[str] = Field(
        default=None, description="Pre-existing access token (alternative to username/password)"
    )

    # Server Settings
    server_name: str = Field(default="care-mcp-server", description="MCP server name")
    ...
    def has_credentials(self) -> bool:
        ...
        return bool(self.care_access_token or (self.care_username and self.care_password))
